...
***/
void FirstBuildSelectionTree(int nTree[], int nData[], int nNdx);
...
void main()
{
	...
	***/
	FirstBuildSelectionTree(nSelectionTree, nRunData, 1);
...
{
	if (nNdx >= RUNctr) {
		nTree[nNdx] = nNdx - RUNctr;
		nData[nNdx - RUNctr] = rand() % DataRANGE + 1000;
	}
	else {
		FirstBuildSelectionTree(nTree, nData, 2 * nNdx);
		FirstBuildSelectionTree(nTree, nData, 2 * nNdx + 1);
		int nNdxL = nTree[2 * nNdx], nNdxR = nTree[2 * nNdx + 1];
		nTree[nNdx] = nData[nNdxL] < nData[nNdxR] ? nNdxL : nNdxR;
	}
}
